...
- `assets/tracks.json` is generated by `build_track_catalog()`, now in
  `scripts/sitegen/feeds.py` (the generator was split out of `scripts/build.py`
  into a `sitegen` package; `build.py` is a thin entry point that imports it —
  `python3 scripts/build.py` still triggers it), same pattern as
  `assets/search-index.json`. One entry per curated track:
  - `id` — `{show-slug}-{tracknum:02d}`, e.g. `jerry-19-broadway-2001-01-08-01`
  - `title` — song title
  - `num` — track number within its show (int)
  - `song` — the canonical song slug (same grouping as the `/songs/` matrix),
    used to keep at most one performance of a song in a generated queue —
    see Phase 2
  ...
  - `flac` / `flac_size_mb` — R2 FLAC key and size, added later for the
    per-song/per-show ZIP download buttons (not in the original sketch)
...
- Page generated by `build_playlist()` in `scripts/sitegen/pages.py`; logic in
  ...
  the sitemap).
- **(2026-07-17) "How to build a playlist" help.** A collapsed-by-default
  `<details class="tech-details pl-help">` between the intro paragraph and
  the presets, explaining both entry points (filter-here-and-generate, vs.
  the Phase 5 "+" while browsing) — added because the two paths and how they
  interact weren't discoverable from the UI alone.
- **(2026-07-17) Shuffle became a real on/off toggle.** The now-playing
  bar's shuffle button previously just reshuffled the remaining queue once
  per click with no state to show for it. It's now persistent: turning it
  on snapshots the current order and randomizes what's left to play, filling
  the button solid (`aria-pressed`, same convention as the Phase 5 "+"/"✓");
  turning it off restores that exact snapshot and re-locates the now-playing
  track in it. Reset wherever a wholly new queue replaces the old one
  (Generate, a preset, a fresh share link, endless-mode's rollover). Mirrored
  in `scripts/continuous-player.js` for the Phase 7 popup player — the two
  are independent implementations by design (see Phase 7), so a shuffle fix
  needs both files touched.
...
- **Phase 5d (2026-07-17): whole-show "+" on `/archive/`.** The archive
  listing only had show-level rows — no per-track markup for "select all"'s
  DOM-scan approach to scope over, and rendering all ~680 tracks' worth of
  hidden buttons (×4, once per artist/date × all/split view) just to reuse
  it would have been disproportionate. Instead `show_add_button()`
  (`scripts/sitegen/fragments.py`) bakes the show's full id list into
  `data-ids` at build time (`show["tracks"]` is already known there) — one
  small button per split show (~120 total across the four view blocks) that
  `track-select.js` toggles as a unit. True toggle, not "select all"'s
  add-only semantics: if every one of the show's tracks is already selected,
  clicking removes them all; otherwise it tops up whatever's missing — so it
  reads correctly even if some of a show's songs were already picked
  individually elsewhere. The button nests inside the row's own `<a>` (a
  second instance of the "interactive-in-interactive" issue Phase 5's
  `.pl-row` change first ran into); `stopPropagation()` keeps its click from
  also following the row's link.
...
- **6b — curated playlists.** Top-level `"playlists"` key in
  ...
  audio_process status --write, update_tracks, make_stream_mp3) round-trips
  unknown top-level keys losslessly. (`stamp_added_dates`, also checked at
  the time, was a one-off migration script since retired.)
