...
Replace the site's four independent audio players with one component,
rendered at different densities depending on context. Add a client-side
loudness control and a small set of other player functions along the way.
...
or transient-cap, same tradeoff already being managed show-by-show in the
audio pipeline). A client-side control sidesteps that ceiling entirely
instead of re-fighting it per track. See "why −19 on-disk doesn't work well"
— worth re-deriving/recording here in full if this plan moves forward.
...
- Files in scope: `scripts/player.js` (shared playback logic), `scripts/site.css`
  / `scripts/home.css` (two separate token systems the component needs to work
  under — see root `CLAUDE.md`'s styling section), and the page generators in
  `scripts/sitegen/` that currently emit each of the four player markups.
- Loudness gain chain: `GainNode` → `DynamicsCompressorNode` (as limiter) →
  destination. Limiter settings need real tuning, not defaults — threshold
  near −1 to −0.1 dB, ratio ~15:1–20:1, hard knee, fast attack (~0 ms floor),
  fast release (tens of ms).
- Other functions:
...
- **Sticky playback across page navigation.** Consolidating the player does
  **not** by itself make playback survive clicking to another page. The site
  is a static multi-page site (`scripts/build.py` generates full separate
  HTML pages) — every internal link is a full page load, which tears down
  all JS state including any playing `<audio>` element. This is true of the
  *current* four-player setup too; `BroadcastChannel` only coordinates
  pause-the-other-one across tabs, it has never kept audio alive across a
  navigation.
...
- **Loudness control default/options.** −20 / −16 / −14 were illustrative
  numbers in the mockup, not yet validated against real listening.

## 6. Implementation Steps

- [ ] Codex review — log findings in `player-consolidation-codex-review.md`
      in this folder
- [ ] Decide on sticky-navigation scope (in this plan or a separate one)
- [ ] Confirm loudness control default/options with real numbers
- [ ] Scope the actual implementation (which files, in what order)
- [ ] Build the compact (track row) density first — smallest surface area,
      used in the most places
- [ ] Build the hero density
- [ ] Build the persistent mini bar
- [ ] Migrate `/playlist/` and `/player/` onto the new component, retire the
      old markup
