...
| **`audio_process.py`** | The engine underneath: `diagnose` (per-track measurements + clipping/dropout/bandwidth/channel verdicts), `plan` (dry run — every track's normalization decision, no audio written; see [Loudness normalization](#loudness-normalization)), `process` (the −20 LUFS loudnorm + tagged FLAC/MP3 + provenance sidecar), `verify` (R2 MD5s vs sidecar), `retag` (retro-fit tags without touching the audio), `status`/`history`/`versions` (what's been done to what, per track), `version-map` (archive-wide: which workflow version every show's tracks are on, flags shows whose tracks span more than one — also browsable at [/archive-data/](/archive-data/)). |
...
| **`batch_process.py`** | Overnight bulk stager for processing every not-yet-done track-listed show unattended — pulls, validates, diagnoses, and processes to local masters + provenance, but deliberately stops before R2/Drive/git (those stay a human-reviewed morning step). A different workflow from the one-show-at-a-time steps above; see `AUDIO_PROCESSING.md` for its four-phase design. |
...
| **`update_tracks.py`** | A lighter re-upload for an already-listed show: matches local files to existing tracks by leading number, overwrites the R2 keys, refreshes size/duration. **Does not** update the provenance sidecar or MD5-verify — for re-processing an already-published show, prefer re-running `publish_show.py prepare`/`publish` against its slug (works on existing shows too, and is what actually produced the v6/v7 reprocesses in Part 5). Keep this one for a quick file swap where provenance doesn't need to change. |
| **`make_stream_mp3.py`** | For **whole-show** (not split-track) recordings: transcodes the lossless original to a 320k MP3 stream proxy on R2 (via the same `encode_mp3_with_qa()` true-peak-trim/verify/checksum QA the per-track pipeline uses) and records it as `stream` (plus `stream_md5`) in the catalog, so the lossless file can stay behind the download gate. Idempotent/resumable — safe to re-run. |
