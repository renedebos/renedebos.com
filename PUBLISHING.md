...
| **`make_stream_mp3.py`** | For **whole-show** (not split-track) recordings: transcodes the lossless original to a 192k MP3 stream proxy on R2 and records it as `stream` in the catalog, so the lossless file can stay behind the download gate. Idempotent/resumable — safe to re-run. |
